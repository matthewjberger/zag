...
pub fn tables() -> Tables {
    ...
    declare_field(&mut tables, header, b"magic", word, 0);
    declare_field(&mut tables, header, b"version", half, 4);
    declare_field(&mut tables, header, b"flags", half, 6);
    declare_field(&mut tables, header, b"length", word, 8);
